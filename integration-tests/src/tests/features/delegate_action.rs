...
use near_chain_configs::{Genesis, NBC_BASE};
...
const INITIAL_ALLOWANCE: Balance = NBC_BASE;
...
fn setup_with_access_key(
    ...
    add_account_with_access_key(&mut genesis, sender.clone(), NBC_BASE, public_key, access_key);
...
fn meta_tx_create_implicit_account(new_account: AccountId) {
    ...
    let initial_amount = match new_account.get_account_type() {
        AccountType::NearImplicitAccount => NBC_BASE,
