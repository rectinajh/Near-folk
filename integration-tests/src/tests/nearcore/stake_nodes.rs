...
use near_chain_configs::{Genesis, NBC_BASE, TrackedShardsConfig};
use near_client::{
    ...
    heavy_test(|| {
        ...
        run_actix(async {
            let test_nodes = init_test_staking(
                ...
                15,
                false,
                (TESTING_INIT_STAKE / NBC_BASE) as u64 + 1,
                false,
            );
            let mut rng = rand::thread_rng();
            let stakes = (0..num_nodes / 2).map(|_| NBC_BASE + rng.gen_range(1..100));
