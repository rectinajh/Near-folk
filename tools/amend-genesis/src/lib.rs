...
use near_chain_configs::{Genesis, GenesisValidationMode, NBC_BASE};
        ...
        <S as SerializeSeq>::Error: Send + Sync + 'static,
    {
        match self.account {
            Some(mut account) => {
                ...
                if self.amount_needed {
                    account.set_amount(10_000 * NBC_BASE);
                }
...
mod test {
    use anyhow::Context;
    use near_chain_configs::{Genesis, GenesisConfig, NBC_BASE, get_initial_supply};
    use near_primitives::account::AccountContract;
    use near_primitives::shard_layout::ShardLayout;
    use near_primitives::state_record::StateRecord;
    use near_primitives::types::{AccountId, AccountInfo};
